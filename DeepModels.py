...
class DeepModels:
    ...
    def data_prep(self, X_train, y_train, X_test, y_test):
        # First we want to convert our dataset to Torch tensors
        ...
        self.Din = X_train.shape[1]

        return train_inputs, train_targets, test_inputs, test_targets

    def SimpleNet(self, num_hidden_layers=3, num_nodes=256):
        ...
        if num_hidden_layers == 1:
            ...
            model = nn.Sequential(
                nn.Linear(self.Din, num_nodes),
                nn.ReLU(),
                nn.Linear(num_nodes, self.Dout)
            )
    ...
    def train_net(self, model, optimizer, criterion, train_inputs, train_targets, test_inputs, test_targets, n_epochs=100):
        ...
        for it in range(n_epochs):
            ...
            test_acc = self._accuracy(model, test_inputs, test_targets)
    ...
    def _accuracy(self, model, test_inputs, test_targets):
...
if __name__ == "__main__":
    ...
    dm = DeepModels()
    train_inputs, train_targets, test_inputs, test_targets = dm.data_prep(etl.X_train, etl.y_train, etl.X_test, etl.y_test)
    model = dm.SimpleNet(num_hidden_layers=1, num_nodes=128)
    optimizer, criterion = dm.optimization_params(model, optimizer_name='Adam', lr=0.001, momentum=0.99)
    dm.train_net(model, optimizer, criterion, train_inputs, train_targets, test_inputs,
                                              test_targets, n_epochs=100)

    print(dm.test_accuracies)